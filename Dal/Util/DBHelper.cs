...
namespace VipCard.Dal.Util
{
    public class DBHelper
    {
        ...
        public static IDictionary<string, object> QueryOneDicRow(string sql, params object[] args)
        {
            //查询单行记录，委托方法
            IList<IDictionary<string, object>> rows = QueryDicRows(sql, args);
            //查询到一行记录的情况，正确期待值
            if (rows.Count == 1)
            {
